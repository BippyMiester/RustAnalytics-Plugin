...
/**
 * Under the MIT license, we are redistributing this plugin under a different name as to avoid plugin conflicts with playtime rewards being reset at the end of every wipe, or whenever a server owner decides to wipe the data. This data for RustMetricsPlaytimeTracker should NEVER be wiped. You don't have to do any configuration at all with this plugin. Just drop it in with RustMetricsPro. If you chose to wipe this data, then your stats on your dashboard on RustMetrics.pro for players playtime will be inaccurate as it will reset the playtime to 0 for all users that connect. So just don't touch this plugin.
 * 
 * Original Plugin created by k1lly0u, and can be downloaded under the MIT license at the following link: https://umod.org/plugins/playtime-tracker
 */

namespace Oxide.Plugins
{
    [Info("RustAnalyticsPlaytimeTracker", "k1lly0u", "0.2.2")]
    [Description("Track player time spent on the server")]
    class RustAnalyticsPlaytimeTracker : CovalencePlugin
    {
        #region Fields        
